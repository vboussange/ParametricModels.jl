...
@testset "InferenceResult" begin
    ...
    dudt = Modelα(ModelParams(N=N))

    res = construct_result(dudt,MiniBatchInference.ResultMLE())
    @test res.m.mp.N isa Number
end

@testset "InferenceResult with Composable model" begin
    N = 10
    tspan = (0., 1.)
    g = complete_graph(N)
    lap_mf = Matrix{Float32}(laplacian_matrix(g)) ./ Float32(N)
    ...
    dudt = Modelαμ(ModelParams(N=N,lap=lap_mf,uworld = t -> zeros(N)))
    cm = ComposableModel((dudt,dudt,))

    res = Econobio.construct_result(cm, MiniBatchInference.ResultMLE(p_trained = zeros(2*length(dudt))))
    @test res.m.mp.N isa Number
end
